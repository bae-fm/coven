//! HTTP-backed `CloudHome` implementation (the ShareProxy backend).
//!
//! Talks to a share-proxy server's `/cloud/*` write proxy endpoints.
...
/// HTTP-backed cloud home that proxies through a share-proxy server.
pub struct ShareProxyCloudHome {
    base_url: String,
    keypair: UserKeypair,
    client: Client,
    clock: ClockRef,
...
#[async_trait]
impl CloudHome for ShareProxyCloudHome {
    async fn write(
        &self,
        key: &str,
        data: Vec<u8>,
        ...
        // this device. The server PUT is a single request — there's no resumable
        // protocol to the server — but a streaming body lets reqwest pull one
        ...
        // is set explicitly so the server sees a sized request rather than
    ...
    async fn grant_access(&self, _member_id: &str) -> Result<CloudHomeJoinInfo, CloudHomeError> {
        Ok(CloudHomeJoinInfo::ShareProxy {
...
mod tests {
    ...
    fn test_cloud_home(base_url: &str, keypair: UserKeypair) -> ShareProxyCloudHome {
        ShareProxyCloudHome::new(
            base_url.to_string(),
            keypair,
    ...
    fn grant_access_returns_join_info() {
        ...
        match join_info {
            ...
            _ => panic!("expected ShareProxy join info"),
        }
    }
