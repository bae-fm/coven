...
pub(crate) const CACHE_BUDGET_STATE_KEY_PREFIX: &str = "cache_budget:";
...
pub(crate) fn cache_budget_state_key(namespace: &str) -> String {
...
pub(crate) async fn drop_cached_blob(
    db: &StoreDatabase,
    store_dir: &StoreDir,
...
/// `pinned/`. [`crate::sync::store::blob::StoreBlobCache::pin`] dispatches this per
/// blob, up to its concurrency limit, and takes `&[RowBlobRef]` rather than ids
/// because every operation must use and revalidate the exact row version and
/// locator. `remote` is evaluated only after both local cache locations miss.
pub(crate) async fn pin_one<'a, R, F>(
    db: &StoreDatabase,
    store_dir: &StoreDir,
    remote: R,
    reference: &RowBlobRef,
...
/// failed delete.
#[cfg(test)]
pub(crate) async fn clear_cache(store_dir: &StoreDir) -> Result<(), BlobCacheError> {
...
pub(crate) async fn evict_to_budget(
    db: &StoreDatabase,
    store_dir: &StoreDir,
    namespace: &str,
