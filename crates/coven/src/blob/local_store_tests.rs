//! Tests for the local store: coven's own copy of a
...
async fn store_read_drop_round_trip() {
    ...
    let read = ld
        .read_local_blob("covers", "cov0aaaa", bytes.len() as u64)
        .await
        .expect("read");
    ...
    assert_eq!(
        ld.read_local_blob("covers", "absent00", bytes.len() as u64)
            .await
            .unwrap(),
        None,
    ...
    assert_eq!(
        ld.read_local_blob("covers", "cov0aaaa", bytes.len() as u64)
            .await
            .unwrap(),
        None,
...
async fn local_store_blob_survives_an_evict_to_budget_sweep() {
    ...
    ld.store_local_blob("covers", "keep0aaa", &store_bytes)
        .await
    ...
    cache.enforce_budget("audio", None).await.expect("evict");
    ...
    assert_eq!(
        ld.read_local_blob("covers", "keep0aaa", store_bytes.len() as u64)
            .await
            .unwrap(),
        Some(store_bytes),
