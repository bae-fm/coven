...
impl CircleBootstrapRef {
    pub(crate) fn verify_for_access(&self, access: &CircleAccessLeaf) -> bool {
        if crate::protocol::store_commit::validate_commit_frontier(&self.coverage).is_err() {
            return false;
        }
        ...
        let semantic_prefix = crate::protocol::store_commit::circle_bootstrap_image_semantic_prefix(
            access.circle_id,
