...
pub(super) fn validate_access_pairs(
    ...
    while index < objects.len() {
        ...
        let envelope_material = materials
            ...
            .ok_or(RemoteObjectRecordError::CandidateObjectMissing)?;
        let leaf: crate::protocol::circle_control::CircleAccessLeaf =
        ...
        let envelope: crate::protocol::circle_control::AccessEnvelope =
        ...
        let bootstrap = match &leaf.disposition {
            crate::protocol::circle_control::CircleAccessDisposition::Active {
                bootstrap, ..
            } => bootstrap.as_ref(),
            crate::protocol::circle_control::CircleAccessDisposition::Inactive => None,
        };
        match bootstrap {
            Some(bootstrap) => {
...
pub(super) fn validate_package_reference(
    ...
    circle: Option<&crate::protocol::store_commit::CirclePackageRef>,
    canonical_semantic_bytes: &[u8],
    object: &ExactObjectRef,
) -> Result<(), RemoteObjectRecordError> {
    let package =
        crate::protocol::audience_package::AudiencePackage::parse(canonical_semantic_bytes)
            .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
    ...
    match (circle, package.audience()) {
        (None, crate::protocol::audience_package::PackageAudience::Store) => Ok(()),
        (
            Some(reference),
...
pub(super) fn validate_circle_epoch_close_intent_identity(
    ...
    let intent: crate::protocol::circle_control::CircleEpochCloseIntent =
        serde_json::from_slice(canonical_semantic_bytes)
            .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
    ...
    let expected = format!(
        "{}.json",
        crate::protocol::circle_control::circle_epoch_close_intent_semantic_prefix(
            circle_id,
            reference.close_id,
...
pub(super) fn validate_circle_epoch_close_outcome_identity(
    ...
    let crate::protocol::circle_control::CircleEpochCloseSlotValue::Outcome(outcome) =
        crate::protocol::circle_control::CircleEpochCloseSlotValue::parse(canonical_semantic_bytes)
            .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?
    else {
        return Err(RemoteObjectRecordError::StoredReferenceMismatch);
    };
    let expected = format!(
        "{}.json",
        crate::protocol::circle_control::circle_epoch_close_outcome_semantic_prefix(
            circle_id,
            reference.close_id,
        )
    );
...
pub(super) fn validate_circle_epoch_close_cancellation_identity(
    ...
    let crate::protocol::circle_control::CircleEpochCloseSlotValue::Cancellation(cancellation) =
        crate::protocol::circle_control::CircleEpochCloseSlotValue::parse(canonical_semantic_bytes)
            .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?
    else {
        return Err(RemoteObjectRecordError::StoredReferenceMismatch);
...
pub(super) fn validate_retained_authority_identity(
    ...
    match &identity.domain {
        ...
        RetainedAuthorityObjectDomain::Acknowledgement { reference } => {
            let acknowledgement: crate::protocol::store_commit::StoreAck =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
        ...
        RetainedAuthorityObjectDomain::CircleAcknowledgement { reference } => {
            let acknowledgement: crate::protocol::store_commit::CircleAck =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
            if acknowledgement.registration != reference.registration
        ...
        RetainedAuthorityObjectDomain::MergeMembershipWrappedStoreKey { reference } => {
            let wrapped: crate::protocol::wrapped_store_key::WrappedStoreKey =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
            reference
        ...
        RetainedAuthorityObjectDomain::StoreMembershipResolution { reference } => {
            ...
            let expected_key = format!(
                "{}.json",
                crate::protocol::store_commit::membership_resolution_semantic_prefix(
        ...
        RetainedAuthorityObjectDomain::MergeMembershipEntry { reference } => {
            let entry: crate::protocol::membership::MembershipEntry =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
        ...
        RetainedAuthorityObjectDomain::MergeMembershipHead { reference } => {
            let head: crate::protocol::membership::AuthorHead =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
        ...
        RetainedAuthorityObjectDomain::DeviceExclusionProposal { reference } => {
            let proposal: crate::protocol::store_commit::StoreDeviceExclusionProposal =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
            reference
        ...
        RetainedAuthorityObjectDomain::DeviceExclusionOutcome { reference } => {
            let outcome: crate::protocol::store_commit::StoreDeviceExclusionOutcome =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
        ...
        RetainedAuthorityObjectDomain::ReclaimEvidence { reference } => {
            let value: crate::protocol::reclaim::ReclaimEvidence =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
            reference
                .verify(&value)
        ...
        RetainedAuthorityObjectDomain::ReclaimAuthorization { reference } => {
            let value: crate::protocol::reclaim::ReclaimAuthorization =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
            reference
        ...
        RetainedAuthorityObjectDomain::ReclaimReceipt { reference } => {
            let value: crate::protocol::reclaim::ReclaimReceipt =
                serde_json::from_slice(canonical_semantic_bytes)
                    .map_err(|error| RemoteObjectRecordError::InvalidDomain(error.to_string()))?;
            reference
                .verify_identity(&value)
