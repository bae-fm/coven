...
pub(crate) fn exact_logical_object(
    logical_key: String,
    bytes: &[u8],
...
pub(crate) fn merge_membership_ref(
    owner: &UserKeypair,
...
pub(crate) struct MergeDeviceAuthority {
    pub(crate) registration: store_commit::StoreDeviceRegistration,
    pub(crate) reference: store_commit::StoreDeviceRegistrationRef,
    pub(crate) device_signer: UserKeypair,
    pub(crate) stream_id: membership::AuthorStreamId,
}

impl MergeDeviceAuthority {
    pub(crate) fn circle_control_reference(
        &self,
...
pub(crate) fn merge_device_authority(
