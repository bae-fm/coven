...
impl MembershipChain {
    ...
    fn from_entries_with_coords_and_head_refs(
        ...
        provider_admin_genesis: crate::protocol::provider::ProviderAdminState,
    ) -> Result<Self, MembershipError> {
        if entries.is_empty() {
    ...
    fn rebuild(&mut self) -> Result<(), MembershipError> {
        ...
        let (state_source, status) = match reduced {
            CausalGrantStatus::Resolved(reduced) => {
                let provider_admin = crate::protocol::provider::ProviderAdminState::reduce_merge(
                    provider_admin_seed,
                    &self.entries,
                    &reduced.included,
                )?;
                ...
                let branches = maximal_valid_branches
                    ...
                    .map(|branch| -> Result<StoreMembershipBranch, MembershipError> {
                        let resolved = resolved_store_membership(
                            &branch.reduced,
                            checkpoint_grants,
                            crate::protocol::provider::ProviderAdminState::reduce_merge(
                                provider_admin_seed,
                                &self.entries,
