...
pub(crate) struct RowBlobLocatorBinding {
    table: String,
    row_id: String,
    row_stamp: String,
    column: String,
...
impl RowBlobLocatorBinding {
    ...
    pub(crate) fn row_stamp(&self) -> &str {
        &self.row_stamp
    }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct AudiencePackage {
    version: u32,
...
impl AudiencePackage {
    ...
    pub(crate) fn parse(bytes: &[u8]) -> Result<Self, AudiencePackageError> {
    ...
    pub(crate) fn candidate_family(&self) -> CandidateFamilyId {
        self.candidate_family
    }

    pub(crate) fn commit_coord(&self) -> &StoreCommitCoord {
        &self.commit_coord
    }
    ...
    pub(crate) fn audience(&self) -> &PackageAudience {
        &self.audience
    }
...
mod tests {
    ...
    fn circle_package_refuses_locator_from_another_key() {
        ...
        let wrong_key_locator = BlobLocator::opaque(
            ...
            BlobScope::Master,
            KeyFingerprint::from_bytes([5; 32]),
            7,
