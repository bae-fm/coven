...
pub(crate) struct MembershipCausalFloor {
...
impl MembershipCausalFloor {
    ...
    pub(crate) fn advance(
        &mut self,
        coordinate: crate::protocol::membership::MembershipCoord,
...
impl RetainedVerifiedActivatedAck {
    pub(crate) fn latest(&self) -> Option<&(StoreAckRef, StoreAck)> {
        self.chain
            .last_key_value()
            .map(|(_, acknowledgement)| acknowledgement)
    }

    pub(crate) fn exactly_extends(&self, predecessor: &Self) -> bool {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct RetainedAcceptedStoreAnnouncement {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct RetainedMergeMembershipProof {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct RetainedVerifiedMergeHistorySummary {
    pub version: u32,
...
impl RetainedVerifiedMergeHistorySummary {
    pub(crate) fn digest(&self) -> ObjectHash {
    ...
    pub(crate) fn validate_shape(&self) -> Result<(), StoreProtocolError> {
        ...
        for (reference, proof) in &self.membership_proofs {
            ...
            if transition.body.entry != proof.entry
                ...
                || !crate::protocol::membership::verify_membership_entry(&proof.entry_value)
            {
                return Err(StoreProtocolError::DeviceStateMismatch);
            }
            proof
            ...
            match (
                ...
                (
                    crate::protocol::membership::MembershipChange::ResolutionActivation {
                        resolution,
                    },
                    Some(reference),
                    Some(value),
    ...
    pub(crate) fn validate_snapshot_baseline(&self) -> Result<(), StoreProtocolError> {
        self.validate_shape()?;
