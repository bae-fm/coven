...
pub(crate) fn store_protocol_root_logical_key() -> &'static str {
    STORE_PROTOCOL_ROOT_SEMANTIC_PATH
}

pub(crate) fn device_join_attempt_semantic_prefix(attempt_id: DeviceJoinAttemptId) -> String {
    format!("{STORE_DEVICE_JOIN_ATTEMPT_PREFIX}{attempt_id}")
}

pub(crate) fn circle_access_leaf_semantic_prefix(
    circle_id: CircleId,
...
pub(crate) fn circle_access_envelope_semantic_prefix(
    circle_id: CircleId,
    family: CandidateFamilyId,
    owner_pubkey: &str,
    recipient_slot: &str,
...
pub(crate) fn device_join_outcome_semantic_prefix(attempt_id: DeviceJoinAttemptId) -> String {
    format!("{STORE_DEVICE_JOIN_OUTCOME_PREFIX}{attempt_id}")
}

pub(crate) fn device_join_abandonment_semantic_prefix(attempt_id: DeviceJoinAttemptId) -> String {
    device_join_attempt_semantic_prefix(attempt_id)
}

pub(crate) fn device_join_cleanup_receipt_semantic_prefix(
    attempt_id: DeviceJoinAttemptId,
) -> String {
    format!("{STORE_DEVICE_JOIN_CLEANUP_RECEIPT_PREFIX}{attempt_id}")
}

pub(crate) fn device_exclusion_proposal_semantic_prefix(
...
pub(crate) fn device_exclusion_outcome_semantic_prefix(
    target: StoreDeviceId,
    proposal_id: StoreDeviceExclusionProposalId,
) -> String {
...
pub(crate) fn package_semantic_prefix(
    family: CandidateFamilyId,
    device_id: &str,
    seq: u64,
...
pub(crate) fn circle_package_semantic_prefix(
    circle_id: CircleId,
    family: CandidateFamilyId,
    device_id: &str,
    seq: u64,
...
pub(crate) fn circle_bootstrap_image_semantic_prefix(
    circle_id: CircleId,
    family: CandidateFamilyId,
    owner_pubkey: &str,
    epoch_id: CircleEpochId,
...
pub(crate) fn commit_slot_prefix(device_id: &str, seq: u64) -> String {
    format!("{STORE_CANDIDATE_PREFIX}*/commits/{device_id}/{seq}")
}

pub(crate) fn commit_semantic_prefix(
    family: CandidateFamilyId,
    device_id: &str,
    seq: u64,
...
pub(crate) fn semantic_prefix_from_exact_object(
    object: &ExactObjectRef,
    extension: &str,
...
pub(crate) fn head_slot_prefix(device_id: &str, seq: u64) -> String {
...
pub(crate) fn registration_semantic_prefix(device_id: &str) -> String {
    registration_slot_prefix(device_id)
}

pub(crate) fn founder_registration_semantic_prefix(creation_id: StoreCreationId) -> String {
    format!("store-v1/devices/founder/{creation_id}/registration")
}

pub(crate) fn founder_membership_head_semantic_prefix(creation_id: StoreCreationId) -> String {
    format!("{STORE_MEMBERSHIP_HEAD_PREFIX}founder/{creation_id}/1")
}

pub(crate) fn ack_slot_prefix(device_id: &str, revision: u64) -> String {
    format!("{STORE_ACK_PREFIX}{device_id}/{revision}")
}

pub(crate) fn circle_ack_slot_prefix(
    circle_id: CircleId,
    device_id: &str,
    sequence: u64,
) -> String {
...
pub(crate) fn circle_snapshot_slot_prefix(
    circle_id: CircleId,
    device_id: &str,
    generation: u64,
) -> String {
...
pub(crate) fn snapshot_slot_prefix(device_id: &str, generation: u64) -> String {
    format!("{STORE_SNAPSHOT_META_PREFIX}{device_id}/{generation}")
}

pub(crate) fn membership_entry_semantic_prefix(
    author: &str,
...
#[cfg(test)]
pub(crate) fn membership_head_semantic_prefix(
    author: &str,
    author_owner_grant: &MembershipGrantId,
    stream_id: AuthorStreamId,
    seq: u64,
...
pub(crate) fn membership_head_slot_prefix(
    author: &str,
    author_owner_grant: &MembershipGrantId,
    stream_id: AuthorStreamId,
    seq: u64,
...
pub(crate) fn membership_resolution_semantic_prefix(
...
pub(crate) fn snapshot_image_semantic_prefix(author: &str, image_hash: ObjectHash) -> String {
