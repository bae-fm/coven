...
fn fixture() -> Fixture {
    ...
    let membership =
        crate::protocol::membership::MembershipChain::from_entries(vec![founder.clone()])
            .expect("founder membership");
    let crate::protocol::membership::MembershipStatus::Resolved(resolved_membership) =
        membership.status()
    else {
        panic!("founder membership resolves")
    };
    let membership_head = crate::protocol::membership::MembershipHeadRef {
        coord: founder.coord(),
    ...
    let commit = StoreBatchCommit::signed_operations(
        ...
        StoreOperationMembershipAuthority {
            predecessor: crate::protocol::membership::MembershipGrantCreationAuthority::Entry(
                founder.coord(),
            ),
        },
        StoreCommitOperationsInput {
...
fn readiness_rejects_a_bootstrap_cut_other_than_the_signed_attempt_cut() {
    ...
    let access_grant = crate::protocol::provider::StoreMemberProviderAccessGrant::signed(
        access_grant_id,
        keys::public_key_hex(&joiner),
        provider_admin.provider.clone(),
    ...
    .unwrap();
    let access_grant_ref = crate::protocol::provider::StoreMemberProviderAccessGrantRef::from_grant(
        &access_grant,
        exact(
...
/// A same-principal join attempt and a cancellation of it, with the terminals a
/// cleanup receipt has to carry. Both malleability tests below sign a receipt
/// over this and then re-encode its slot list.
struct CleanupFixture {
    ...
    access_locator: crate::protocol::provider::ProviderAccessLocator,
}

fn cleanup_fixture(fixture: &Fixture, joiner: &UserKeypair) -> CleanupFixture {
    use crate::protocol::store_commit::device_join_exchange as exchange;

    let attempt_id = DeviceJoinAttemptId::from_hash(ObjectHash::digest(b"cleanup attempt"));
    let attempt_slot = slot("store-v1/device-join-attempts/cleanup.json".to_string());
    let outcome_slot = slot("store-v1/device-join-outcomes/cleanup.json".to_string());
    let registration_slot = slot("store-v1/device-registrations/cleanup-joiner.json".to_string());
    let provider_admin = crate::protocol::provider::ProviderAdminState::founder_from_root(
        fixture.root_ref.clone(),
        fixture.registration_ref.clone(),
        &fixture.root.descriptor.founder_provider_admin,
    ...
    let registration = StoreDeviceRegistration::signed(
        ...
        provider_admin.provider.clone(),
        DeviceStreamAnchor::StoreAnnouncements {
            first_slot: slot("store-v1/heads/cleanup-joiner/1.json".to_string()),
        },
        DeviceStreamAnchor::StoreAcknowledgements {
            first_slot: slot("store-v1/acks/cleanup-joiner/1.json".to_string()),
        },
        DeviceStreamAnchor::StoreSnapshots {
            first_slot: slot("store-v1/snapshots/cleanup-joiner/1.json".to_string()),
        },
        joiner,
    )
    .unwrap();
    let membership = StoreMembershipStateRef::from_parts(
        ...
        Vec::new(),
        Vec::new(),
        ObjectHash::digest(b"cleanup membership"),
    )
    .unwrap();
    let owner_device_signer = fixture.registration.device_signer(&fixture.signer).unwrap();
    let offer = exchange::DeviceJoinOffer::signed(
        attempt_id,
        keys::public_key_hex(joiner),
        fixture.root_ref.clone(),
        fixture.root.descriptor.provider.clone(),
    ...
    let attempt = DeviceJoinAttempt::signed(
        ...
        provider_admin.grant_id.clone(),
        approval,
        exchange::DeviceProviderResponseReservation::SamePrincipal,
        fixture.registration_ref.clone(),
        fixture.root.descriptor.founder_grant.clone(),
        &fixture.registration,
        &owner_device_signer,
    ...
    let cancellation = crate::protocol::store_commit::DeviceJoinOutcomeRef::Cancelled {
        attempt: attempt_ref,
        outcome_hash: ObjectHash::digest(b"cleanup outcome"),
        object: exact(
            "store-v1/device-join-outcomes/cleanup.json".to_string(),
            b"cleanup outcome",
        ),
    };
    let administrator_terminal = exchange::ProviderAdminJoinTerminal::Cancelled(
        exchange::ProviderAdminJoinClosure::signed(
            cancellation.clone(),
            fixture.registration_ref.clone(),
            exchange::ProviderChallengeDisposition::SamePrincipal,
            ObjectHash::digest(b"administrator prior state"),
            &fixture.registration,
            &owner_device_signer,
        )
        .unwrap(),
    );
    let joiner_device_signer = registration.device_signer(joiner).unwrap();
    let joiner_terminal = exchange::JoinerJoinTerminal::Cancelled(
        exchange::JoinerJoinClosure::signed(
            cancellation.clone(),
            registration,
            exchange::SlotDisposition::NeverCreated,
            exchange::SlotDisposition::NeverCreated,
            exchange::JoinerResponseDisposition::SamePrincipal,
            ObjectHash::digest(b"joiner prior state"),
            &joiner_device_signer,
        )
        .unwrap(),
    );
    ...
    CleanupFixture {
        ...
        joiner_terminal,
        membership,
        provider_admin_grant: provider_admin.grant_id,
        access_locator: provider_admin.access,
    }
}

#[test]
fn a_cleanup_receipt_with_unsorted_deleted_slots_is_refused() {
    use crate::protocol::store_commit::device_join_exchange as exchange;

    let fixture = fixture();
    let joiner = UserKeypair::generate();
    let parts = cleanup_fixture(&fixture, &joiner);
    let owner_device_signer = fixture.registration.device_signer(&fixture.signer).unwrap();
    let canonical = exchange::canonical_cleanup_slots(&parts.attempt).unwrap();
    assert!(
        canonical.len() > 1,
        "the malleability needs at least two slots to reorder"
    );

    let receipt = exchange::DeviceJoinCleanupReceiptObject::signed(
        &parts.attempt,
        parts.cancellation.clone(),
        parts.administrator_terminal.clone(),
        parts.joiner_terminal.clone(),
        canonical.clone(),
        parts.membership.clone(),
        parts.provider_admin_grant.clone(),
        fixture.registration_ref.clone(),
        &fixture.registration,
        &owner_device_signer,
    )
    .expect("sign a canonical cleanup receipt");
    assert!(receipt
        .verify(&parts.attempt, &fixture.registration)
        .is_ok());
    ...
    assert!(
        reordered
            .verify(&parts.attempt, &fixture.registration)
            .is_err(),
        "a receipt whose deleted slots are not in canonical order must not verify",
    );
}

#[test]
fn a_write_revocation_with_unsorted_protected_slots_is_refused() {
    use crate::protocol::store_commit::device_join_exchange as exchange;
    ...
    let owner_device_signer = fixture.registration.device_signer(&fixture.signer).unwrap();
    let slots = vec![
        slot("store-v1/protected/a.json".to_string()),
        slot("store-v1/protected/b.json".to_string()),
    ];
    ...
    let revocation = exchange::DeviceJoinProducerWriteRevocation::signed(
        ...
        slots,
        crate::protocol::provider::ProviderAccessWithdrawal::Direct {
    ...
    .expect("sign a canonical write revocation");
