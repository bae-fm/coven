...
fn table_row_count(connection: &Connection, table: DatabaseTestTable) -> Result<i64, DbError> {
    connection
...
fn clear_table(connection: &Connection, table: DatabaseTestTable) -> Result<(), DbError> {
    connection
...
fn author_exclusion_activation_evidence(
...
/// Test-only inspection and corruption of a standalone SQLite database image.
///
/// The connection is retained here so snapshot and failed-bootstrap tests never
/// open or pass SQLite connections outside the database boundary.
