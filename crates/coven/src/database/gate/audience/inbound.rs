...
pub(super) fn winning_store_audience(
    conn: &Connection,
    routing_id: &str,
...
pub(super) type PackageRoutes = HashMap<(String, String), (String, String)>;
...
pub(super) unsafe fn filter_inbound_audience_rows_raw(
    conn: &Connection,
    changeset: &[u8],
    package_audience: &Audience,
    gates: &Gates,
...
pub(super) unsafe fn normalize_inbound_private_routes_raw(
    conn: &Connection,
    changeset: &[u8],
    package_audience: &Audience,
    store_transitions: &StoreAudienceTransitions,
...
pub(super) unsafe fn validate_inbound_private_routes_raw(
    conn: &Connection,
    changeset: &[u8],
    package_audience: &Audience,
    store_transitions: &StoreAudienceTransitions,
        ...
        if row.table != "_coven_row_routes" {
            if gates.table_is_scoped(&row.table) && row.op == ffi::SQLITE_INSERT {
                let row_id = row
                    .pk()
                    .ok_or_else(|| GateError::MissingChangesetPrimaryKey(row.table.clone()))?;
                let columns = crate::database::gate::gate_table_columns(conn, &row.table)?;
