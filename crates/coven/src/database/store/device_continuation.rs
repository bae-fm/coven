...
impl StoreDatabase {
    ...
    pub(crate) async fn export_activated_device_continuation(
    ...
    ) -> Result<crate::restore_code::ActivatedContinuation, DbError> {
        let durable = self
        ...
        Ok(crate::restore_code::ActivatedContinuation {
            ...
            .call(move |conn| {
                ...
                match existing_ack {
                    ...
                    1 => {
                        ...
                        {
                            return Err(DbError::Message(
                                "restored acknowledgement successor differs from its signature"
                                    .into(),
                            ));
                        }
                    }
                ...
                match existing_device {
                    Some(existing)
                        if existing == continuation.registration.device_id.to_string() => {}
                    Some(_) => {
                        return Err(DbError::Message(
