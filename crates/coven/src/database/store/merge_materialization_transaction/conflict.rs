...
//! ([`crate::protocol::hlc::MAX_FUTURE_SKEW_MS`]) and refuses to let a grossly-future one win
...
impl TableSchema {
    ...
    pub(crate) fn updated_at(&self, table: &str) -> Option<usize> {
        self.updated_at_by_table.get(table).copied()
    }

    pub(crate) fn columns(&self, table: &str) -> Option<&[String]> {
...
/// stamp — beyond `receiver_wall_ms` + [`crate::protocol::hlc::MAX_FUTURE_SKEW_MS`] — is
