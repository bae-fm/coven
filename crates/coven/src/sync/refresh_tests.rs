...
use crate::sync::test_helpers::{
    capture_bytes, open_test_db, pubkey_hex, temp_store_dir, test_store_security, TestCustody,
    TestStore,
};
...
async fn removal_rotation_stays_resumable_when_local_adoption_fails() {
    ...
    // local removal journal and its gate for the exact removal retry to complete.
    {
        ...
        assert!(matches!(
            ...
            Err(crate::storage::RotationPendingState::LocalCommitted { generation: 2 })
        ));
    }
