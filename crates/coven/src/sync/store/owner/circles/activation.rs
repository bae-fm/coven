...
async fn load_verified_epoch_close_intent(
    ...
    .await?;
    let intent: crate::protocol::circle::CircleEpochCloseIntent = serde_json::from_slice(&bytes)
        .map_err(|error| {
...
async fn verify_epoch_close_outcome(
    ...
    for (participant, settlement_ref) in close.participants.iter().zip(&outcome.responses) {
        ...
        let slot_value = crate::protocol::circle::CircleEpochCloseResponseSlotValue::parse(&bytes)
            .map_err(|error| {
                CircleOperationError::InvalidState(format!(
...
async fn verify_epoch_reopen(
    ...
    let cancellation_prefix = crate::protocol::circle::circle_epoch_close_outcome_semantic_prefix(
        control.value.circle_id,
        close.close_id,
    );
    ...
    let crate::protocol::circle::CircleEpochCloseSlotValue::Cancellation(cancellation) =
        crate::protocol::circle::CircleEpochCloseSlotValue::parse(&cancellation_bytes).map_err(
            |error| {
                CircleOperationError::InvalidState(format!(
