...
async fn replay_circle_roster_resolutions(
    ...
    let current_head_refs =
        crate::protocol::circle::CircleRosterChain::validate_exact_heads(entries, current_heads)
            .map_err(|error| CircleOperationError::InvalidState(error.to_string()))?;
    ...
    for resolution in resolutions {
        ...
        .await?;
        crate::protocol::circle::CircleRosterChain::validate_exact_heads(&conflict_entries, &heads)
            .map_err(|error| CircleOperationError::InvalidState(error.to_string()))?;
