...
pub(crate) struct AuthorizedMembershipRevocation<'operation, 'storage, 'input> {
    ...
    pending_rotation: &'input cloud_storage::PendingRotation,
...
impl<'operation, 'storage, 'input> AuthorizedMembershipRevocation<'operation, 'storage, 'input> {
    ...
    pub(crate) async fn begin(
        ...
        let permit = operation.database.membership_mutation_permit().await;
        Self {
            operation,
            chain,
            revokee_pubkey,
    ...
    async fn build_revoke_mutation(
        ...
        let owner_keypair = self.operation.writer.identity.clone();
        ...
        let publication = if chain.is_owner_now(revokee_pubkey) {
            ...
            let transition = AuthorizedMembershipPublication::new(self.operation)
            ...
            .await
            .map_err(|error| InviteError::InvalidDurableMutation(error.to_string()))?;
            let head = AuthorizedMembershipPublication::new(self.operation)
                .finish_transition(
                    transition.clone(),
            ...
            RevokeMembershipPublication::Direct {
                publication: Box::new(
                    AuthorizedMembershipPublication::new(self.operation)
    ...
    pub(crate) async fn execute(mut self) -> Result<EncryptionService, InviteError> {
            ...
            .await?
        {
            Some(row) => {
                ...
                if !plan.matches_request(&owner_keypair, self.revokee_pubkey, self.store_id) {
                    return Err(InviteError::PendingMutation(
            ...
            &progress
        {
            ...
            persistence
                .finish_nonactivating_revoke(&plan, operation.storage.cloud_home())
                .await?;
            ...
            RevokeMembershipPublication::StoreActivated {
                ...
                loop {
                    ...
                    crate::sync::store::operations::StoreOperationPublicationOutcome::NonactivatedCandidate {
                        ...
                        persistence
                            .begin_nonactivating_revoke(
                                &plan,
                                operation.storage.cloud_home(),
                                verified,
                            )
                            .await?;
                        pending_rotation
