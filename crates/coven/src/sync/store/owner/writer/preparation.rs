...
pub(super) struct StoreWritePreparer<'operation, 'storage, 'directory> {
    operation: &'operation mut AuthorizedWriterOperation<'storage>,
    store_dir: &'directory StoreDir,
    database: crate::database::StoreDatabase,
    pending: PreparedStoreWrite,
}

impl<'operation, 'storage, 'directory> StoreWritePreparer<'operation, 'storage, 'directory> {
    pub(super) fn new(
        operation: &'operation mut AuthorizedWriterOperation<'storage>,
        store_dir: &'directory StoreDir,
        database: crate::database::StoreDatabase,
    ...
    pub(super) async fn run(self) -> Result<bool, StoreError> {
        let Self {
            operation,
            store_dir,
            database,
