...
async fn durable_descriptor_reservation(
    ...
    let probes = StoreCreationProbeIds {
        exact_slots: crate::protocol::provider::ProviderProbeId::from_bytes(
    ...
    let initialized = StoreCreationAttempt::Initialized(StoreCreationAuthority {
        ...
        provider_admin_grant: crate::protocol::provider::ProviderAdminGrantId::from_random_bytes(
            crate::encryption::generate_random_key(),
        ),
        probes,
    ...
    let allocation_context = crate::storage::ProtocolObjectContext::signed_plaintext(
...
async fn durable_founder_graph_reservation(
    ...
    let device = crate::protocol::store_commit::StoreDeviceId::derive(root, &origin).to_string();
...
pub(super) async fn prepare_founder_graph(
    ...
    let root_context = crate::storage::ProtocolObjectContext::signed_plaintext(
        root_hash,
        ProtocolObjectDomain::StoreProtocolRoot,
    );
