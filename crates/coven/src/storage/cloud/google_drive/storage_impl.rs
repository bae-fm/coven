...
pub(super) fn parse_drive_file_identities(
...
pub(super) fn select_drive_file(files: &[DriveFileIdentity]) -> Option<&DriveFileIdentity> {
...
pub(super) fn parse_create_file_id(body: &str, key: &str) -> Result<String, CloudHomeError> {
...
pub(super) fn parse_generated_file_id(
...
pub(super) fn create_file_metadata_body(
    encoded_name: &str,
    folder_id: &str,
    create_token: &str,
) -> String {
...
pub(super) struct DriveMultipartSink<'a> {
...
#[async_trait]
impl crate::storage::cloud::PartSink for DriveMultipartSink<'_> {
...
/// that JSON.
pub(super) fn parse_google_api_error_reason(body: &str) -> Option<String> {
...
pub(super) fn classify_write_error(
...
pub(super) const GDRIVE_SIMPLE_UPLOAD_MAX: usize = 4 * 1024 * 1024;
...
pub(super) const GDRIVE_CHUNK_SIZE: usize = 8 * 1024 * 1024;
...
impl OAuthRestHome for GoogleDriveCloudHome {
    ...
    async fn send_read(
        ...
        let range = range.map(|(start, end)| crate::storage::cloud::range_header(start, end));
        self.session
            .api_call(|oauth| {
                let mut req =
...
impl ExactSlotStorage for GoogleDriveCloudHome {
    ...
    async fn read_range_at(
        ...
        let range = crate::storage::cloud::range_header(start, end);
...
pub(super) fn drive_permissions_next_page_url(
    list_url: &str,
    page: &serde_json::Value,
