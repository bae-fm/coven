...
/// base object key.
pub(super) fn strip_part_suffix(key: &str) -> &str {
...
impl ChunkManifest {
    pub(super) fn new(total_len: usize, upload_id: String) -> Self {
        Self {
...
pub(super) fn encode_chunk_manifest(manifest: ChunkManifest) -> Vec<u8> {
...
pub(super) fn chunk_manifest_key(key: &str) -> String {
    format!("{key}{CHUNK_MANIFEST_SUFFIX}")
}

pub(super) fn chunk_part_key(key: &str, upload_id: &str, index: usize) -> String {
    format!("{key}.part{index}.{upload_id}")
}

pub(super) fn decode_chunk_manifest(data: &[u8]) -> Result<ChunkManifest, CloudHomeError> {
...
pub(super) struct CloudKitStagingCleanup {
    pub(super) ops: Arc<dyn CloudKitOps>,
    pub(super) scope: CloudKitScope,
    pub(super) batch: CloudKitAtomicCreateBatch,
    pub(super) armed: std::sync::atomic::AtomicBool,
}
...
impl CloudKitStagingCleanup {
    ...
    pub(super) fn cleanup_failure(&self, operation: CloudHomeError) -> CloudHomeError {
        self.disarm();
...
pub(super) enum AtomicCreateReadback {
    Created(Vec<CloudKitRecordVersion>),
    Absent,
}

pub(super) fn parse_chunk_key(key: &str, upload_id: &str) -> Result<Option<usize>, CloudHomeError> {
...
pub(super) fn list_numbered_chunks(
    ops: &dyn CloudKitOps,
    scope: &CloudKitScope,
    key: &str,
...
pub(super) fn verify_chunk_manifest(
    key: &str,
    manifest: &ChunkManifest,
    chunks: &[(usize, String)],
) -> Result<(), CloudHomeError> {
...
pub(super) fn missing_or_unassembled(
    ops: &dyn CloudKitOps,
    scope: &CloudKitScope,
    key: String,
) -> CloudHomeError {
...
pub(super) fn read_chunked_object(
    ops: &dyn CloudKitOps,
    scope: &CloudKitScope,
    key: &str,
    manifest: ChunkManifest,
...
pub(super) fn delete_chunk_layout(
    ops: &dyn CloudKitOps,
    scope: &CloudKitScope,
    key: &str,
) -> Result<(), CloudHomeError> {
...
pub(super) fn delete_stale_chunk_records(
    ops: &dyn CloudKitOps,
    scope: &CloudKitScope,
    key: &str,
    upload_id: &str,
...
pub(super) fn delete_single_record(
    ops: &dyn CloudKitOps,
    scope: &CloudKitScope,
    key: &str,
) -> Result<(), CloudHomeError> {
...
pub(super) fn delete_all_variants(
    ops: &dyn CloudKitOps,
    scope: &CloudKitScope,
    key: &str,
) -> Result<(), CloudHomeError> {
