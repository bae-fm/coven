...
pub struct DurablePreparedProtocolObject {
    pub semantic_bytes: Vec<u8>,
    pub prepared: PreparedExactObject,
}

impl DurablePreparedProtocolObject {
    pub fn new(semantic_bytes: Vec<u8>, prepared: PreparedExactObject) -> Self {
        Self {
            semantic_bytes,
            prepared,
        }
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct StoreBatchLocalCleanup {
    pub drops: Vec<coven_protocol::blob::DeferredLocalBlobDrop>,
}

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct StoreBatchCompletion {}

pub(crate) fn validate_host_synced_tables(
    conn: &Connection,
...
pub(crate) fn canonical_table_name(
    conn: &Connection,
    table: &str,
...
pub(crate) fn validate_existing_row_identities(
    conn: &Connection,
    table: &SyncedTable,
) -> Result<(), DbError> {
    if table.row_identity() == coven_protocol::synced_schema::RowIdentity::SharedKey {
        return Ok(());
    }
    let sql = format!("SELECT id FROM {}", crate::quote_ident(table.name()));
...
/// Enforce the synced-table contract ([`coven_protocol::synced_schema::SyncedTable`]) on
...
pub(crate) fn validate_synced_table_contract(
    conn: &Connection,
    table: &str,
) -> Result<(), DbError> {
...
pub(crate) fn declared_as_text(declared_type: &str) -> bool {
...
pub(crate) fn table_is_strict(conn: &Connection, table: &str) -> Result<Option<bool>, DbError> {
    let sql = format!("PRAGMA table_list({})", crate::quote_ident(table));
