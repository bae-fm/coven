...
impl StoreSession<'_> {
    ...
    > {
        let Some(commit) =
            StoreDatabase::circle_activation_commit_ref_on(self.records.conn, circle_id, control)?
        else {
            return Ok(None);
        };
    ...
    > {
        ...
        let commit =
            StoreDatabase::circle_activation_commit_ref_on(self.records.conn, circle_id, &head)?
                .ok_or_else(|| {
                    DbError::Message(format!(
