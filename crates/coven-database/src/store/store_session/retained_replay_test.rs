...
/// A baseline image carries the rows that name payloads, never the payloads
...
#[test]
fn the_generation_zero_image_carries_no_payload_rows() {
    let source = Connection::open_in_memory().expect("open projection fixture");
    populate_fixture(&source);
    let transaction = source.unchecked_transaction().expect("begin installation");
    let hash = crate::payload_store::write_payload_blocking(&transaction, b"a payload")
        .expect("install a payload");
    crate::payload_store::set_payload_owner_claims_on(
        &transaction,
        "owner",
        &BTreeSet::from([hash]),
    )
    .expect("claim the payload");
    transaction.commit().expect("commit installation");
    assert!(
        !crate::payload_store::payload_rows_in_image(&source)
            .expect("count the source's payload rows")
            .is_empty(),
        "the fixture holds payloads for the projection to shed"
    );

    let bytes = project_generation_zero_image(&source)
        .expect("project the generation-zero image")
        .database_bytes()
        .expect("serialize the generation-zero image");

    let mut image = Connection::open_in_memory().expect("open the projected image");
    crate::connection_io::deserialize_database_image_into(&mut image, &bytes)
        .expect("load the projected image");
    assert_eq!(
        crate::payload_store::payload_rows_in_image(&image)
