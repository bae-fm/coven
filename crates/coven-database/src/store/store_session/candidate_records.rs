...
pub(super) fn parse_prepared_merge_candidate_on(
    ...
    authority.prepared_merge_candidate_on(records, prepared)
}

pub(super) fn prepared_merge_candidate_objects(
    prepared: &PreparedStoreWriteState,
) -> (
...
pub(super) fn parse_prepared_merge_candidate_parts_on(
    ...
    authority.prepared_merge_candidate_parts_on(
        records,
        commit_bytes,
        commit_object,
        head_bytes,
...
pub(super) fn verify_prepared_merge_candidate_parts(
    root: &coven_protocol::store_commit::StoreRootRef,
    unverified: StoreBatchCommit,
...
pub(super) fn author_exclusion_activation_for_candidate_on(
    ...
    select_author_exclusion_activation_locator(
        ...
        &expected_stream,
        *sequence,
        |exclusion| load_author_exclusion_activation_locator_on(records, retained, root, exclusion),
    )
}
...
pub(super) fn load_author_exclusion_activation_locator_on(
    ...
    let retained =
        retained.retained_materialization_by_ref_on(records, locator.activation_commit())?;
    if retained.root() != root {
        return Err(DbError::Message(
            "author exclusion activation belongs to another Store root".to_string(),
        ));
    }
...
pub(super) fn terminal_candidate_verification_on(
    ...
    let authority = match proof {
        coven_protocol::remote_object::CandidateNonactivationProof::AuthorExclusion {
            ...
            load_author_exclusion_activation_locator_on(records, retained, root, exclusion)?,
        ),
        coven_protocol::remote_object::CandidateNonactivationProof::MergeMembershipGrantRevocation {
            grant_id,
            membership,
...
impl crate::store::store_session::StoreTransaction<'_, '_> {
    pub(super) fn prepared_merge_candidate(
        ...
        parse_prepared_merge_candidate_on(self.records, authority, prepared)
    }
    ...
    pub(super) fn prepared_merge_publication(
        ...
        parse_prepared_merge_publication_on(self.records, authority, prepared)
    }
    ...
    pub(super) fn load_author_exclusion_activation_locator(
        ...
        load_author_exclusion_activation_locator_on(self.records, retained, root, exclusion)
    }
    ...
    pub(super) fn validate_terminal_nonactivation_authority(
        ...
        validate_terminal_nonactivation_authority_on(self.records, retained, root, durable)
    }
    ...
    pub(super) fn validate_terminal_candidate_authority(
        ...
        validate_terminal_candidate_authority_on(self.records, retained, root, candidate, durable)
    }
}
