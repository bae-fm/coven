...
pub struct RetainedReplayGenesisAuthority {
...
impl RetainedReplayBaseline {
    ...
    fn validate_image_metadata(&self, image: &Connection) -> Result<(), DbError> {
        ...
        let stored_routing_hash =
            crate::required_protocol_state_on(image, SYNC_ROUTING_HASH_STATE_KEY)?;
...
pub(crate) fn copy_table_with_conflicts(
    ...
    let quoted_columns = columns
        ...
        .collect::<Vec<_>>()
        .join(", ");
    let select = format!("SELECT {quoted_columns} FROM {}", crate::quote_ident(table));
...
pub(super) fn project_generation_zero_image(
    ...
    for table in crate::user_table_names(&transaction).map_err(DbError::from)? {
