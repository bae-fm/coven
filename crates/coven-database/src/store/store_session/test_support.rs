...
impl StoreSession<'_> {
    ...
    fn persist_prepared_audience_objects_for_test(
        ...
        crate::DatabaseTestSql::for_store(self.conn, self.store_dir).transaction(|transaction| {
            ...
            transaction.persist_prepared_audience_objects(write_id, packages, blobs)
        })
    }
    ...
    fn import_prepared_write(
        ...
        for (expected_hash, bytes) in &transfer.payloads {
            let actual_hash = crate::payload_store::write_payload_blocking(&transaction, bytes)?;
    ...
    fn insert_write_status_for_test(
        ...
        let transaction = self.conn.unchecked_transaction().map_err(DbError::from)?;
        let changeset_hash = crate::payload_store::write_payload_blocking(&transaction, b"")?;
