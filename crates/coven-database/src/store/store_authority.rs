        ...
        self.connection
            .call_store(move |session| {
                ...
                if authority.creation_id != descriptor.creation_id
                    ...
                    || descriptor.founder_membership.first_slot()
                        != &reservation.membership.first_slot
                {
                    return Err(DbError::Message(
                        "signed Store descriptor differs from its durable creation attempt"
                            .to_string(),
                    ));
                }
                if graph.registration.value.store_commits != graph_reservation.store_commits
                    || graph.registration.value.acknowledgements
                        != graph_reservation.acknowledgements
                    ...
                    || graph.initial_ack.value.successor.next_slot
                        != graph_reservation.next_ack_slot
                ...
                {
                    return Err(DbError::Message(
                        "signed founder graph differs from its durable slot reservation"
                            .to_string(),
                    ));
                }
                ...
                for key in [exact_key, STORE_CREATION_ATTEMPT_STATE_KEY.to_string()] {
                    ...
                    if deleted != 1 {
                        return Err(DbError::Message(
                            "Store creation journal disappeared during typed consumption"
                                .to_string(),
                        ));
                    }
                }
        ...
        self.connection.call_store(move |session| {
            ...
            match &graph.registration_state {
                ...
                LocalDeviceRegistrationState::Activated { authority } => {
                    ...
                    let installed = verified_authority.root_authority_on(
            ...
            verified_authority
                .commit_installed_registration(registration, graph.registration.value);
            Ok(())
        })
        .await
    }
