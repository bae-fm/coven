...
pub struct SyncRoutingContract {
    bytes: Vec<u8>,
    hash: ObjectHash,
...
fn canonical_unique_parent_keys(
    ...
    let sql = format!("PRAGMA index_list({})", crate::quote_ident(parent_table));
    ...
    for (index_name, unique, partial) in indexes {
        ...
        let sql = format!("PRAGMA index_xinfo({})", crate::quote_ident(&index_name));
