...
use crate::circle_control::CircleControlCoord;
...
fn row_blob_reference_rejects_locator_facts_from_another_blob() {
    ...
    let authority = RowBlobAuthority::Remote(crate::audience_package::PackageAudience::Circle {
...
fn browsable_row_blob_reference_requires_its_declared_cloud_path() {
    ...
    assert!(row_ref(
        ...
        plaintext_size,
        plaintext_hash,
        RowBlobAuthority::Remote(crate::audience_package::PackageAudience::Store),
        stored(wrong_path),
    )
    .is_err());
}
