...
/// [`SyncedTable::gated_through`]. [`SyncedTable::remote_root`]
...
impl SyncedTable {
    ...
    /// Gate this plain table through the parent row its `column` foreign key
    /// points to: a row is visible wherever that parent row is, whatever kind of
    /// root the chain ends at — a boolean gated root, an audience root, a remote
    /// root, or a kept ancestor. Every plain table with a foreign key into a
    /// gated table must name this relationship; coven never chooses among the
    /// table's foreign keys.
    pub fn gated_through(mut self, column: impl Into<String>) -> Self {
