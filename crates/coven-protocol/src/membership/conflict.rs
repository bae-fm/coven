...
impl MembershipConflictChoice {
    ...
    pub fn conflict_hash(&self) -> ObjectHash {
        self.conflict_hash
    }

    pub fn selection(&self) -> &MembershipConflictSelection {
        &self.selection
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct StoreMembershipConflictResolutionBody {
...
pub type StoreMembershipConflictResolution = Signed<StoreMembershipConflictResolutionBody>;
    ...
    let (mut grants, known_records, provider_admin) = match conflict {
        ...
        MembershipConflict::RevocationCycle {
            ...
            let known_records = maximal_valid_branches
                ...
                .map(|(grant, state)| (grant.clone(), state.record().clone()))
                .collect::<BTreeMap<_, _>>();
            let provider_admin = crate::provider::ProviderAdminResolution::Resolved(
                crate::provider::ProviderAdminState::merge(
                    selected_branches
                        .iter()
