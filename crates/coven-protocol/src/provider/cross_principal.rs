...
pub fn validate_cross_provider_evidence_context(
    store: &StoreProviderBinding,
    context: &CrossPrincipalChallengeContext,
) -> Result<(), ProviderProbeError> {
    context
...
pub fn validate_cross_provider_evidence(
    store: &StoreProviderBinding,
    ...
    let compatible = matches!(
        ...
        (
            StoreProviderBinding::GoogleDrive {
                corpus: crate::objects::GoogleDriveCorpus::SharedDrive { .. }
            },
    ...
    {
        let crate::objects::ProviderPrincipalId::CloudKitSharedZoneParticipant { record_name } =
            &peer.principal
        else {
