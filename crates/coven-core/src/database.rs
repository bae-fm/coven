...
fn load_store_device_exclusion_freezes_on(
    conn: &Connection,
    ...
    for row in rows {
        ...
        let StoreHistoryCut::MergeConcurrent(target_frontier) = &target_cut else {
            return Err(DbError::Message(
...
fn apply_store_device_exclusion_freezes_on(
    conn: &Connection,
    root: &crate::sync::store_commit::StoreRootRef,
    policy: WritePolicy,
    ...
    if let Some(local_registration) = local_activated_registration_ref_on(conn)? {
        for (reference, proposal) in operations.proposals() {
            ...
            desired.push(StoreDeviceProposalAck {
                ...
                target_cut: StoreHistoryCut::MergeConcurrent(match target {
...
fn validate_initialized_coven_schema(
    conn: &Connection,
...
impl DatabaseCore {
    ...
    fn state(&self) -> DatabaseState {
        DatabaseState {
            ...
            store_device_exclusion: Arc::new(tokio::sync::Mutex::new(())),
...
fn load_author_exclusion_activation_locator_on(
    ...
    let locator = AuthorExclusionActivationLocator {
        ...
        activation_commit: serde_json::from_str(&activation_commit).map_err(|error| {
...
fn validate_terminal_nonactivation_authority_on(
    conn: &Connection,
    durable: &crate::sync::remote_object::CandidateNonactivation,
) -> Result<(), DbError> {
    match durable.proof() {
...
impl Database {
    ...
    pub(crate) fn merge_materialization_failure_injection(
        ...
        MergeMaterializationFailureInjection {
            armed: self.state.merge_materialization_failure.clone(),
        }
    }
    ...
    pub(crate) async fn retained_merge_materialization(
        ...
        self.call(move |conn| Self::load_retained_merge_materialization_by_ref_on(conn, &reference))
            .await
    }
    ...
    pub(crate) async fn retained_merge_history_frontier(
        ...
        self.call(move |conn| {
            references
                .iter()
                .map(|reference| Self::load_retained_merge_history_checkpoint_on(conn, reference))
                .collect()
        })
        .await
    }
    ...
    pub(crate) async fn resolved_store_device_state(
        ...
        self.call(move |conn| load_declared_store_device_state_on(conn, &reference))
            .await
    }
    ...
    fn load_retained_merge_materialization_by_ref_on(
        conn: &Connection,
        reference: &StoreBatchCommitRef,
    ) -> Result<OwnedVerifiedMergeMaterialization, DbError> {
        let StoreCommitCoord::MergeConcurrent {
    ...
    fn load_retained_merge_history_checkpoint_on(
        conn: &Connection,
        reference: &StoreBatchCommitRef,
...
fn local_activated_registration_ref_on(
    conn: &Connection,
...
fn store_device_exclusion_journal_error(error: StoreDeviceExclusionJournalError) -> DbError {
    DbError::Message(error.to_string())
}
...
fn load_store_device_exclusion_on(
    conn: &Connection,
    operation_id: ObjectHash,
...
fn load_active_store_device_exclusion_on(
    conn: &Connection,
) -> Result<Option<DurableStoreDeviceExclusionOperation>, DbError> {
    conn.query_row(
...
fn insert_store_device_exclusion_on(
    conn: &Connection,
    operation: &DurableStoreDeviceExclusionOperation,
    active: bool,
) -> Result<(), DbError> {
...
fn require_store_device_exclusion_transition_on(
    conn: &Connection,
...
fn update_store_device_exclusion_on(
    conn: &Connection,
    expected: &DurableStoreDeviceExclusionOperation,
    next: &DurableStoreDeviceExclusionOperation,
    active: bool,
