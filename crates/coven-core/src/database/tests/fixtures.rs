...
pub(super) fn reclaim_test_activation(
...
pub(super) fn notes_migration() -> Migration {
    Migration::sql(
        1,
        "notes",
        "CREATE TABLE notes (
...
pub(super) fn things_migration() -> Migration {
    Migration::sql(
        1,
        "things",
        "CREATE TABLE things (
...
pub(super) fn scoped_things_migration() -> Migration {
    Migration::sql(
        1,
        "scoped things",
        "CREATE TABLE things (
...
pub(super) fn local_row_blob(row_id: &str, stamp: &str, bytes: &[u8]) -> RowBlobRef {
...
pub(super) fn open_outbox_database(device_id: &str) -> Database {
    Database::open(
        Path::new(":memory:"),
        Vec::new(),
...
pub(super) fn test_candidate_family() -> crate::sync::store_commit::CandidateFamilyId {
