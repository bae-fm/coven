...
fn require_merge_candidate(candidate: &PreparedMergeStoreOperationCommit) -> Result<(), DbError> {
    if candidate.reference.coord.policy() == WritePolicy::MergeConcurrent {
        Ok(())
    } else {
        Err(DbError::Message(
            "Merge acknowledgement candidate carries a Serial coordinate".to_string(),
        ))
    }
}

impl MergeDatabase<'_> {
    pub(in crate::sync::store_engine) async fn prepare_acknowledgement_activation(
        self,
        expected: StoreAckRef,
        candidate: PreparedMergeStoreOperationCommit,
    ) -> Result<(), DbError> {
        require_merge_candidate(&candidate)?;
        let candidate = PreparedStoreOperationCommit::MergeConcurrent(candidate);
        self.database
            .call(move |conn| {
                ...
                match outbound.activation {
                    ...
                    OutboundStoreAckActivation::Prepared(
                        PreparedStoreOperationCommit::MergeConcurrent(existing),
                    ...
                    | OutboundStoreAckActivation::Nonactivating(
                        ...
                        return Err(DbError::Message(
                            "Merge acknowledgement contains a Serial activation candidate"
                                .to_string(),
                        ));
                    }
    ...
    pub(in crate::sync::store_engine) async fn adopt_acknowledgement_head(
        ...
        self.database
            .call(move |conn| {
                ...
                let OutboundStoreAckActivation::Prepared(
                    PreparedStoreOperationCommit::MergeConcurrent(candidate),
                ) = outbound.activation
                else {
                    return Err(DbError::Message(
                ...
                let mut wrapped = PreparedStoreOperationCommit::MergeConcurrent(candidate);
                wrapped
