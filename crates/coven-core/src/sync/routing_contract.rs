...
use super::session::{foreign_key_edges, GateRole, RowIdentity, SyncedTable};
...
pub(crate) enum SyncRoutingContractError {
    ...
    #[error(transparent)]
    ForeignKey(#[from] super::session::ForeignKeySchemaError),
...
fn live_columns(
    ...
    let sql = format!("PRAGMA table_info({})", super::session::quote_ident(table));
...
fn canonical_unique_parent_keys(
    ...
    let sql = format!(
        "PRAGMA index_list({})",
        super::session::quote_ident(parent_table)
    );
    let mut statement = conn.prepare(&sql)?;
    ...
    for (index_name, unique, partial) in indexes {
        ...
        let sql = format!(
            "PRAGMA index_xinfo({})",
            super::session::quote_ident(&index_name)
        );
        let mut statement = conn.prepare(&sql)?;
        let rows = statement
            .query_map([], |row| {
