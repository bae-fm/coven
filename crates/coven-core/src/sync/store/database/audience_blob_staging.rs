...
use crate::database::{
    load_activated_registration_on, local_activated_registration_ref_on,
    required_store_root_authority_on, DbError, StoreWriteBlobFact, StoreWriteBlobFacts,
    StoreWriteBlobMoveDestination,
};
use crate::store_dir::StoreDir;
use crate::sync::gate::{AudienceMove, AudiencePartition};
use crate::sync::storage::{BlobSpoolProtection, BlobWriteAuthority, SyncStorage};
...
pub struct HostWriteBlobStaging {
    ...
    storage: Result<Option<std::sync::Arc<dyn SyncStorage>>, String>,
    store_dir: StoreDir,
}
...
impl StoreDatabase {
    pub(crate) fn record_prepared_transition_local_blob_moves(
        ...
        for fact in &mut facts.blobs {
            ...
            if audience_move.destination == crate::sync::circle::Audience::Local {
    ...
    pub(crate) fn stage_audience_move_blobs_on(
        ...
        staging: &HostWriteBlobStaging,
    ) -> Result<StagedAudienceBlobFiles, DbError> {
        staging.runtime.block_on(async {
            let mut files = StagedAudienceBlobFiles::new();
            let storage = staging
                .storage
                .as_ref()
                .map_err(|error| DbError::Message(error.clone()))?
                .as_deref();
            let result = Self::stage_audience_move_blobs_inner(
                tx,
                facts,
                moves,
                partitions,
    ...
    pub(crate) fn rollback_staged_audience_blobs(
        ...
        match staging.runtime.block_on(files.rollback()) {
            Ok(()) => operation,
    ...
    async fn stage_audience_move_blobs_inner(
        ...
        let remote_destination_exists = facts.blobs.iter().any(|fact| {
            moved_rows
                ...
                .is_some_and(|audience_move| {
                    audience_move.destination != crate::sync::circle::Audience::Local
                })
        });
        let registration = if remote_destination_exists {
            ...
            let registration = load_activated_registration_on(tx, &root, &reference)?;
        ...
        for fact in &mut facts.blobs {
            ...
            match &audience_move.destination {
                crate::sync::circle::Audience::Local => {
                    stage_local_destination(tx, storage, store_dir, fact, &source, files).await?;
                    fact.audience_move = Some(StoreWriteBlobMoveDestination::Local);
                }
                destination => {
                    let storage = storage.ok_or_else(|| {
                        move_materialization_error(
                            fact,
                            "destination audience requires connected cloud storage",
                        )
                    })?;
                    ...
                    let locator =
                        crate::sync::store::package_preparation::prepare_partition_blob_locator(
                            fact,
                            audience.clone(),
                            &protection,
                            &authority,
                    ...
                    let source_path =
                        move_source_plaintext(tx, storage, store_dir, fact, &source, &spool_path)
                            .await?;
                    let spool_write = storage
                        ...
                        .await
                        .map_err(|error| move_materialization_error(fact, error.to_string()))?;
                    if spool_write == crate::sync::storage::BlobSpoolWrite::Created {
...
fn source_authority(
    ...
    source: &crate::sync::circle::Audience,
) -> Result<RowBlobAuthority, DbError> {
    match source {
        crate::sync::circle::Audience::Local => Ok(RowBlobAuthority::Local),
        source => {
...
fn destination_protection(
    ...
    destination: &crate::sync::circle::Audience,
    partitions: &[AudiencePartition],
    fact: &StoreWriteBlobFact,
) -> Result<(RemoteAudience, BlobSpoolProtection), DbError> {
    match destination {
        crate::sync::circle::Audience::Store => storage
        ...
        crate::sync::circle::Audience::Local => unreachable!("Local handled before protection"),
    }
}
...
async fn move_source_plaintext(
    ...
    let protection =
        crate::sync::store::blob::opening_protection_on(tx, storage, source, &previous.stored)
            .map_err(|error| move_materialization_error(fact, error.to_string()))?;
...
async fn stage_local_destination(
    ...
    let storage = storage.ok_or_else(|| {
        move_materialization_error(fact, "remote source requires connected cloud storage")
    })?;
    let previous = fact.previous.as_ref().ok_or_else(|| {
        move_materialization_error(fact, "remote source has no exact prior blob locator")
    })?;
