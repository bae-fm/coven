use crate::database::*;
use crate::sync::store_commit::{StoreAck, StoreDeviceRegistration, StoreDeviceRegistrationRef};
            ...
            .call(move |conn| {
                ...
                match existing {
                    ...
                    Some(_) => Err(DbError::Message(
                        "local registration journal already owns different exact objects"
                            .to_string(),
                    )),
                    None => conn
                        .execute(
        ...
        self.sqlite()
            .call(move |conn| {
                ...
                if registration_ref.object != *stored_registration_prepared.reference()
                    ...
                    || initial_ack_object.prepared.stored_bytes()
                        != stored_ack_prepared.stored_bytes()
