use crate::database::*;
use crate::sync::store_commit::{StoreAck, StoreAckRef, StoreDeviceRegistrationRef};
...
impl StoreDatabase {
    pub(crate) async fn latest_local_store_ack(
        ...
        self.sqlite().call(load_published_store_ack_on).await
    }
                ...
                .map(|raw| {
                    ...
                    if reference.registration != registration {
                        return Err(DbError::Message(
                            "activated Store acknowledgement names another registration"
                                .to_string(),
                        ));
                    }
                    Ok(reference)
    ...
    pub(crate) async fn stage_store_ack(
        ...
        self.sqlite()
            .call(move |conn| {
                ...
                if verified != ack {
                    return Err(DbError::Message(
                        "staged Store acknowledgement changed during exact verification"
                            .to_string(),
                    ));
                }
    ...
    pub(crate) async fn oldest_outbound_store_ack(
        ...
        self.sqlite().call(load_outbound_store_ack_on).await
    }
