...
/// }
/// ```
pub struct BootstrapResult<'storage> {
    store_id: String,
    target_path: PathBuf,
    db_hash: String,
...
pub fn create_snapshot(
    ...
    create_snapshot_with_host_blobs(conn, root, temp_dir, tables, routing_encryption)
...
fn create_snapshot_for_audience_with_host_blobs(
    ...
    let routing_key = if gates.has_scoped_graph() {
        ...
        let key = crate::sync::circle::derive_row_routing_key(encryption, root.store_root_hash)
            .map_err(|error| SnapshotError::ClearFailed(error.to_string()))?;
        Some(key)
    } else {
        None
...
fn clear_local_only_tables(
    ...
    clear_non_synced(&conn, root, synced, routing_key, audience)?;
    conn.close()
...
fn clear_non_synced(
    ...
    if matches!(audience, crate::sync::circle::Audience::Store) {
        crate::sync::store::StoreDatabase::retain_snapshot_replay_inputs_on(&tx, root)
            .map_err(|error| SnapshotError::ClearFailed(error.to_string()))?;
        crate::sync::store::StoreDatabase::retain_snapshot_device_states_on(&tx, root, coverage)
            .map_err(|error| SnapshotError::ClearFailed(error.to_string()))?;
    }
...
pub async fn bootstrap_from_snapshot<'storage>(
...
mod tests {
    ...
    async fn circle_snapshot_keeps_only_referenced_store_parent_rows() {
        ...
        let image = source
            .call(move |connection| {
                let routing = crate::encryption::EncryptionService::from_key([42; 32]);
                create_circle_snapshot(connection, &root, &image_path, &tables, &routing, circle_id)
                    .map_err(|error| crate::database::DbError::Message(error.to_string()))
            })
            .await
    ...
    async fn snapshot_retains_only_frontier_device_states_without_exclusion_authority() {
        ...
        let image = source
            .call(move |connection| {
                create_snapshot(connection, &root, &image_path, &tables, None)
                    .map_err(|error| crate::database::DbError::Message(error.to_string()))
            })
            .await
            .expect("create scoped snapshot image");
    ...
    async fn run_bootstrap_installs_the_verified_exact_store_root() {
        ...
        let image = source
            .call(move |connection| {
                create_snapshot(connection, &root, &image_path, &image_tables, None)
                    .map_err(|error| crate::database::DbError::Message(error.to_string()))
            })
            .await
        ...
        assert_eq!(
            crate::sync::store::database::StoreDatabase::new(installed.database())
                .local_store_root_ref()
                .await
