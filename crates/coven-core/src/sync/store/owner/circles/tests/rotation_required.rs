...
fn member_storage(fixture: &RotationFixture) -> Arc<crate::sync::cloud_storage::CloudSyncStorage> {
    Arc::new(
...
async fn publish_store_snapshot_cut(
    ...
    authorized
        .push_store_snapshot(
            cut.snapshot,
            cut.coverage,
            authorized.database().sqlite().schema_version(),
