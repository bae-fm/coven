...
async fn propose_device_exclusion(
    ...
    super::propose_device_exclusion(&store_database(db), storage, identity_signer, target).await
}
...
async fn finalize_device_exclusion(
    ...
    super::finalize_device_exclusion(&store_database(db), storage, identity_signer, proposal).await
}
...
async fn cancel_device_exclusion(
    ...
    super::cancel_device_exclusion(&store_database(db), storage, identity_signer, proposal).await
}
...
async fn run_snapshot_preserves_author_exclusion_activation_evidence() {
    ...
    for tamper in [
        ...
        .await;
        Box::pin(crate::sync::store::abandonment::abandon_merge_candidate(
            &store_database(&restored),
            &store.storage,
            &target.device_id.to_string(),
            &signer,
...
async fn run_device_join_bootstrap_records_exclusion_replayed_after_snapshot() {
    ...
    let plan = crate::sync::store::prepare_device_join_bootstrap(
        &store.storage,
        &store.root,
        &replay_cut,
        &activation,
    ...
    let bootstrap = tokio::spawn(async move {
        crate::sync::store::snapshot::bootstrap_from_snapshot(
...
async fn prepare_peer_exclusion(
    ...
    let freezes = store_database(owner_db)
    ...
    let acknowledgement = Box::pin(crate::sync::store::stage_store_acknowledgement_for_test(
        owner_db,
        &store.storage,
        frontier,
    ...
    assert_eq!(
        Box::pin(crate::sync::store::drain_store_acknowledgements_for_test(
            owner_db,
            &store.storage,
            signer,
        ))
...
async fn activate_peer_exclusion(
    ...
    assert!(store_database(owner_db)
        .store_device_exclusion_freezes()
        .await
...
async fn run_excluded_author_candidate_cleanup_case(
    ...
    if cleanup_pending {
        ...
        assert!(matches!(
            ...
            crate::sync::store::abandonment::MergeCandidateAbandonment::NotRequired
                | crate::sync::store::abandonment::MergeCandidateAbandonment::Abandoned
        ));
    }
...
async fn maybe_prepare_merge_abandonment(
    ...
    assert!(Box::pin(
        crate::sync::store::abandonment::prepare_merge_candidate_abandonment(
            &store_database(peer_db),
            &store.storage,
            peer_device_id,
            signer,
    ...
    let is_exact_candidate_hold = |candidate: &StoreBatchCommitRef| {
        matches!(
            ...
            [crate::sync::store::pull::HeldStorePosition {
                ...
                reason: crate::sync::store::pull::HeldStorePositionReason::InactiveDevice {
                    ..
                },
            }] if commit == candidate
        )
...
async fn create_exclusion_test_store(
    ...
    crate::sync::store::ensure_active_registration(&store_database(db), storage)
        .await
