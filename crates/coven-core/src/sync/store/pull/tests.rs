...
#[tokio::test]
async fn one_pull_authenticates_every_reachable_commit_once() {
    let source = crate::sync::test_helpers::open_test_db();
    let identity = crate::keys::UserKeypair::generate();
    let store = crate::sync::test_helpers::TestStore::create(
        &source,
        "single-verification-per-pull",
        identity,
    )
    .await
    .expect("create verification-count Store");
    let schema_version = source.schema_version();

    let founder_first = store
        .publish_changeset(
            "founder",
            1,
            &crate::sync::test_helpers::capture_bytes(
                &crate::sync::test_helpers::open_test_db(),
                &[
                    "INSERT INTO notes (id, title, body, _updated_at, created_at)
                   VALUES ('verification-founder-1', 'founder 1', NULL,
                           '0000000001000-0000-founder', '2026-07-27')",
                ],
            )
            .await,
            schema_version,
        ...
        .await
        .expect("publish first founder commit");
    let writer_first = store
        .publish_changeset(
            "writer",
            1,
            &crate::sync::test_helpers::capture_bytes(
                ...
                &[
                    "INSERT INTO notes (id, title, body, _updated_at, created_at)
                   VALUES ('verification-writer-1', 'writer 1', NULL,
                           '0000000001001-0000-writer', '2026-07-27')",
                ],
            )
            .await,
            schema_version,
        ...
        .await
        .expect("publish first writer commit");

    let target = crate::sync::test_helpers::open_test_db();
    let (_target_temp, target_store_dir) = crate::sync::test_helpers::temp_store_dir();
    let (_, initial_pull) =
        crate::sync::test_helpers::pull_into(&target, &store, &target_store_dir).await;
    assert!(initial_pull.held_positions.is_empty(), "{initial_pull:?}");

    let founder_sequence = store
        .next_commit_sequence("founder")
        .await
        .expect("load next founder sequence");
    let founder_second = store
        .publish_changeset(
            "founder",
            founder_sequence,
            &crate::sync::test_helpers::capture_bytes(
                ...
                &[
                    "INSERT INTO notes (id, title, body, _updated_at, created_at)
                   VALUES ('verification-founder-2', 'founder 2', NULL,
                           '0000000002000-0000-founder', '2026-07-27')",
                ],
            )
            .await,
            schema_version,
        ...
        .await
        .expect("publish second founder commit");
    let writer_sequence = store
        .next_commit_sequence("writer")
        .await
        .expect("load next writer sequence");
    let writer_second = store
        .publish_changeset(
            "writer",
            writer_sequence,
            &crate::sync::test_helpers::capture_bytes(
                ...
                &[
                    "INSERT INTO notes (id, title, body, _updated_at, created_at)
                   VALUES ('verification-writer-2', 'writer 2', NULL,
                           '0000000002001-0000-writer', '2026-07-27')",
                ],
            )
            .await,
            schema_version,
        ...
        .await
        .expect("publish second writer commit");

    let history =
        verify_merge_history_refs(&store.storage, &store.root, [founder_second, writer_second])
            .await
            .expect("load every commit reachable from the remote frontier");
    let references = history.commits.keys().cloned().collect::<Vec<_>>();
    assert!(references.contains(&founder_first));
    assert!(references.contains(&writer_first));
    let audit =
        crate::sync::store_commit::StoreCommitVerificationAudit::begin(references.as_slice());

    let (_, pull) = crate::sync::test_helpers::pull_into(&target, &store, &target_store_dir).await;
...
#[tokio::test]
async fn one_circle_pull_authenticates_every_reachable_commit_once() {
    let member_database = open_scoped_replay_database();
    let (_owner_temp, owner_store_dir) = crate::sync::test_helpers::temp_store_dir();
    let (_member_store_temp, member_store_dir) = crate::sync::test_helpers::temp_store_dir();
    let fixture = effective_access_fixture(
        "single-verification-circle-pull",
        &member_database,
        &owner_store_dir,
        &member_store_dir,
    )
    .await;
    let tip = publish_effective_access_row(
        &fixture,
        &owner_store_dir,
        "one Circle pull",
    ...
    .await;
    let history = verify_merge_history_refs(&fixture.store.storage, &fixture.store.root, [tip])
        .await
        .expect("load every commit reachable from the Circle package");
    let references = history.commits.keys().cloned().collect::<Vec<_>>();
    let audit =
        crate::sync::store_commit::StoreCommitVerificationAudit::begin(references.as_slice());
