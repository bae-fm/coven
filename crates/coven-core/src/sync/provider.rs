...
pub fn canonical_custom_s3_origin(input: &str) -> Result<String, StorageError> {
    if input.ends_with('/') {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct DeviceJoinChallengePublicationRecord {
    pub challenge: CrossPrincipalProbeChallenge,
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case", deny_unknown_fields)]
pub enum DeviceJoinChallengePublicationProgress {
    Prepared,
    Published {
...
#[async_trait]
impl DeviceJoinChallengePublicationJournal for crate::database::Database {
    async fn prepare(
        &self,
        challenge: &CrossPrincipalProbeChallenge,
    ) -> Result<DeviceJoinChallengePublicationRecord, StorageError> {
...
impl ProviderAccessLocator {
    pub fn for_current_administrator(
        ...
        match (&binding.store, &binding.device.principal) {
            (
                ...
                crate::sync::storage::ProviderPrincipalId::CustomS3Credential {
                    access_key_id_hash,
                },
            ...
            (
                StoreProviderBinding::GoogleDrive {
                    corpus: crate::sync::storage::GoogleDriveCorpus::SharedDrive { drive_id, .. },
                },
                crate::sync::storage::ProviderPrincipalId::GoogleDrive { permission_id },
            ...
            (
                ...
                crate::sync::storage::ProviderPrincipalId::Dropbox { account_id },
            ...
            (
                ...
                crate::sync::storage::ProviderPrincipalId::CloudKitPrivateZoneOwner { record_name },
    ...
    pub fn validate_for(
        ...
        let valid = match (store, &provider.principal, self) {
            ...
            (
                StoreProviderBinding::S3 { .. },
                crate::sync::storage::ProviderPrincipalId::Aws { .. },
            ...
            (
                ...
                crate::sync::storage::ProviderPrincipalId::OneDrive { .. },
            ...
            (
                ...
                crate::sync::storage::ProviderPrincipalId::CloudKitSharedZoneParticipant {
                    record_name,
                },
...
#[async_trait]
impl ProviderProbeJournal for crate::database::Database {
    async fn load(
        &self,
        probe_id: ProviderProbeId,
    ) -> Result<Option<ProviderProbeJournalRecord>, StorageError> {
        self.load_provider_probe_journal(probe_id)
            .await
            .map_err(|error| StorageError::Storage(error.to_string()))
    }
    ...
    async fn begin(
        ...
        self.begin_provider_probe_journal(prepared)
            .await
            .map_err(|error| StorageError::Storage(error.to_string()))
    }
    ...
    async fn advance(
        ...
        self.advance_provider_probe_journal(previous.clone(), next)
            .await
            .map_err(|error| StorageError::Storage(error.to_string()))
    }
}
...
fn validate_cross_provider_evidence(
    ...
    {
        let crate::sync::storage::ProviderPrincipalId::CloudKitSharedZoneParticipant {
            record_name,
        } = &peer.principal
        else {
...
mod tests {
    ...
    fn provider_admin_rejects_conflicting_id_reuse() {
        ...
        conflicting.provider = ProviderDeviceBinding {
            principal: crate::sync::storage::ProviderPrincipalId::Aws {
                account_id: "999999999999".to_string(),
                principal: crate::sync::storage::AwsPrincipal::Root,
            },
        };
        assert_eq!(
            state.apply(
    ...
    async fn database_probe_journal_rejects_a_skipped_progress_state() {
        ...
        let binding = crate::sync::storage::ResolvedProviderBinding {
        ...
        assert_eq!(db.begin(prepared.clone()).await.unwrap(), prepared);
        ...
        assert!(db.advance(&prepared, final_record).await.is_err());
        assert_eq!(db.load(probe_id).await.unwrap(), Some(prepared));
    }

    fn admin_record(id: u8, label: &str) -> ProviderAdminGrantRecord {
        let root_object = crate::sync::storage::ExactObjectRef::new(
    ...
    fn test_store_binding() -> crate::sync::storage::StoreProviderBinding {
        crate::sync::storage::StoreProviderBinding::S3 {
            endpoint: crate::sync::storage::S3EndpointBinding::Aws {
    ...
    fn private_cloudkit_binding() -> crate::sync::storage::ResolvedProviderBinding {
        crate::sync::storage::ResolvedProviderBinding {
            store: crate::sync::storage::StoreProviderBinding::CloudKit {
                container_id: "iCloud.example.coven".to_string(),
                environment: crate::sync::storage::CloudKitEnvironment::Development,
                owner_name: "private-owner".to_string(),
                zone_name: "private-zone".to_string(),
            },
            device: crate::sync::storage::ProviderDeviceBinding {
                principal: crate::sync::storage::ProviderPrincipalId::CloudKitPrivateZoneOwner {
                    record_name: "current-user".to_string(),
                },
            },
        }
    ...
    fn test_device_binding() -> crate::sync::storage::ProviderDeviceBinding {
