...
/// The consuming app's OAuth clients. Each Coven builder retains its own
/// value, so unrelated apps in one process never share credentials.
#[derive(Clone, Debug)]
...
impl OAuthClients {
    ...
    pub fn config_for(
        ...
        use crate::cloud::{dropbox, google_drive, onedrive};
    ...
    #[cfg(all(any(test, feature = "test-utils"), feature = "oauth-providers"))]
    ...
    pub(crate) async fn prepare_google_drive(
        ...
        let folder_id = if let Some(id) = existing_folder_id {
            ...
            let response = crate::cloud::supports_all_drives(
                self.client
