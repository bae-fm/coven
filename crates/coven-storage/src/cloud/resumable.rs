...
pub(crate) type ClassifyWrite = Box<dyn Fn(StatusCode, &str) -> CloudHomeError + Send + Sync>;

pub(crate) type CancellationSucceeded = fn(StatusCode) -> bool;
...
pub(crate) struct RangePutUploader {
...
impl RangePutUploader {
    ...
    pub(crate) async fn send_deferred_part(
        &mut self,
        part: Bytes,
        offset: u64,
        is_last: bool,
    ...
    pub(crate) async fn abort(&mut self) -> Result<(), CloudHomeError> {
        if self.completed {
            return Ok(());
        }
...
pub(crate) struct RangePutSink(RangePutUploader);
