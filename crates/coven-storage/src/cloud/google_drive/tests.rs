...
async fn absent_mutable_multipart_body_failure_cancels_the_create_session() {
    ...
    let reader = crate::local_file::PlaintextReader::from_test_reader(FailingMultipartBodyReader {
        emitted: false,
    });
    let body = BlobBody::from_test_reader((GDRIVE_CHUNK_SIZE + 1) as u64, reader);

    let error = home
        .write("mutable/copy", body, &crate::cloud::no_progress())
        .await
...
async fn absent_mutable_multipart_early_eof_cancels_the_create_session() {
    ...
    let reader =
        crate::local_file::PlaintextReader::from_test_reader(EarlyEofMultipartBodyReader {
            emitted: false,
        });
    let body = BlobBody::from_test_reader((GDRIVE_CHUNK_SIZE + 1) as u64, reader);

    let error = home
        .write("mutable/short", body, &crate::cloud::no_progress())
        .await
...
async fn drive_treats_499_as_successful_session_cancellation() {
    ...
    let error = home
        .write("mutable/cancel-499", body, &crate::cloud::no_progress())
        .await
...
async fn canceling_absent_mutable_multipart_cancels_the_create_session() {
    ...
    let reader = crate::local_file::PlaintextReader::from_test_reader(PausedMultipartBodyReader {
        emitted: false,
        waiting: waiting.clone(),
    });
    let body = BlobBody::from_test_reader((GDRIVE_CHUNK_SIZE + 1) as u64, reader);
    let write = tokio::spawn(async move {
        home.write("mutable/copy", body, &crate::cloud::no_progress())
            .await
    });
