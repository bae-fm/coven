...
pub(crate) fn decode_key(filename: &str) -> Result<String, DecodeKeyError> {
    let bytes = hex::decode(filename).map_err(DecodeKeyError::Hex)?;
    String::from_utf8(bytes).map_err(DecodeKeyError::Utf8)
}

pub(crate) fn decode_listed_key(provider: &str, filename: &str) -> Option<String> {
