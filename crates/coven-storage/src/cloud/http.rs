...
pub(crate) async fn ok_bytes(resp: Response, ctx: &str) -> Result<Vec<u8>, CloudHomeError> {
    resp.bytes()
        .await
        .map(|b| b.to_vec())
...
/// being parsed.
pub(crate) async fn ok_json<T: DeserializeOwned>(
    resp: Response,
    ctx: &str,
...
/// loops.
pub(crate) fn range_content_header(start: u64, end: u64, total: u64) -> String {
