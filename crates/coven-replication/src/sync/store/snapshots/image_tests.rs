...
/// A Store that has published one snapshot and adopted it: the bytes that went
/// out to its audience, and the database now standing on the baseline that
/// adoption captured.
async fn published_and_adopted_snapshot(name: &str) -> (Database, Vec<u8>) {
    let store_dir = crate::sync::test_helpers::test_store_dir();
    let source = crate::sync::test_helpers::open_test_db(store_dir.clone());
    let signer = UserKeypair::generate();
    let store = crate::sync::test_helpers::TestStore::create(
    ...
    let published = database
        ...
        .expect("capture snapshot");
    device
        .publish_snapshot(published.clone(), captured_coverage(&database).await)
        .await
        .expect("publish snapshot");
    device
        .stand_on_accepted_snapshot()
...
fn carried_payload_rows(bytes: &[u8]) -> Vec<(&'static str, i64)> {
...
/// A published image is read by devices that resolve payload names in their own
/// catalogs, so it names payloads and carries none.
#[tokio::test]
async fn a_published_store_image_carries_no_payload_rows() {
    let (source, published) = published_and_adopted_snapshot("published-image-payloads").await;
    let (payloads, _) = coven_database::DatabaseImageTest::from_bytes(
        &StoreDatabase::new(&source)
            .database_image_for_test()
            .await
            .expect("serialize the publishing store"),
    )
    .expect("open the publishing store")
    .payload_totals()
    .expect("read the publisher's payload totals");
    assert!(payloads > 0, "the publisher holds payloads to carry");

    assert_eq!(carried_payload_rows(&published), Vec::new());
}

/// The private baseline the adoption captured states the same thing from the
/// other side: it is this device's rewind point, not a second copy of its
/// payload store.
#[tokio::test]
async fn a_captured_replay_baseline_image_carries_no_payload_rows() {
    let (source, _) = published_and_adopted_snapshot("captured-baseline-payloads").await;
    let database = StoreDatabase::new(&source);
    let baseline = database
        .replay_baseline_for_test()
        .await
    ...
    assert!(
        ...
        "adoption captured a baseline over the published snapshot"
    );
    let image = database
        .payload_for_test(baseline.image_payload_hash)
        .await
        .expect("read the captured baseline image");

    assert_eq!(carried_payload_rows(&image), Vec::new());
}

/// And the refusal that keeps it that way, one payload table at a time.
#[tokio::test]
async fn an_image_carrying_payload_rows_is_refused_as_a_baseline() {
    let (source, _) = published_and_adopted_snapshot("refused-carried-payloads").await;

    StoreDatabase::new(&source)
        .assert_replay_baseline_rejects_carried_payload_rows_for_test()
        .await
        .expect("exercise the carried-payload refusal");
}
