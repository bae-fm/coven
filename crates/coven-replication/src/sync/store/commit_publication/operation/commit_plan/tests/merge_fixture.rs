...
impl PreparedWriteFixture {
    ...
    pub(super) fn head_object(&self) -> coven_protocol::objects::ExactObjectRef {
        self.head_object.clone()
    }
    ...
    pub(super) async fn set_write_status(&self, status: coven_protocol::write::WriteStatus) {
        self.database
    ...
    pub(super) async fn retry_blocked_write(
        &self,
    ) -> Result<Vec<coven_protocol::write::WriteId>, coven_database::DbError> {
    ...
    pub(super) async fn discard_blocked_candidate(
        &self,
    ) -> Result<Vec<coven_protocol::write::WriteId>, StoreError> {
        self.device
